...
5. A second LLM summarizes the fetched context into a doctor-friendly patient brief.
6. The response is shown in a chat-like UI.

For MVP, this should be a one-shot interaction, not a full persistent AI chat system.
...
Use a stronger model to summarize the retrieved patient context.
...
- a standalone doctor copilot page
- one backend route
- deterministic DB retrieval
- small model for parsing
- stronger model for summarization
