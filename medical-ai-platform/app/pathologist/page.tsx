...
export default function PathologistPage() {
    ...
    useEffect(() => {
        Promise.all([
            ...
            fetch("/api/cases?sourceRole=pathologist").then(r => r.json()),
    ...
    useEffect(() => {
        ...
        fetch(`/api/hospitals/${selectedHospital.id}/patients?query=${patientQuery}`)
            .then(r => r.json())
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            fetch("/api/cases?sourceRole=pathologist")
                .then(r => r.json())
                .then(d => setRecentCases((d.cases || []).slice(0, 10)));
