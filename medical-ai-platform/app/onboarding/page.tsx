...
type Role = "doctor" | "pathologist" | "patient";
...
const ROLE_CARDS: { role: Role; emoji: string; title: string; description: string }[] = [
    {
        role: "doctor",
        emoji: "🩺",
        title: "Doctor",
        description: "Review assigned cases, generate reports, release results to patients",
    },
    {
        role: "pathologist",
        emoji: "🔬",
        title: "Pathologist",
        description: "Upload diagnostic artifacts and route cases to the care team",
    },
    {
        role: "patient",
        emoji: "🏥",
        title: "Patient",
        description: "View released reports, medications, and care updates",
    },
];
...
export default function OnboardingPage() {
        ...
        fetch("/api/users/me")
            ...
            .then((data) => {
                if (data.user?.isOnboarded) {
                    ...
                    router.push(ROLE_REDIRECT[role] ?? "/patient");
                }
            })
    ...
    async function handleContinue() {
        ...
        try {
            ...
            router.push(ROLE_REDIRECT[selected]);
        } catch (error) {
    ...
    return (
        <div className="min-h-screen grid grid-cols-1 lg:grid-cols-2 bg-[#F6F8F3]">
            ...
            <div className="flex flex-col justify-center px-10 lg:px-20 py-12">
                ...
                <h1 className="text-4xl lg:text-5xl font-semibold mb-3 text-black">
                    How will you use{" "}
                    <span className="font-bold">VaidyaVision</span>?
                </h1>
                <p className="text-gray-500 mb-8 text-base">Choose your role to enter your workspace.</p>
...
function RoleCard({
    ...
    return (
        <button
            onClick={onClick}
            className={`rounded-2xl p-6 border-2 transition-all transform hover:scale-105 text-left ${
                active
