...
def build_voice_graph() -> StateGraph:
    ...
    # All agents feed into build_action_plan
...
def stage_for_confirmation_node(state: GraphState) -> GraphState:
    # Session.set_pending() is called in main.py after graph returns
...
def execute_pending_node(state: GraphState) -> GraphState:
    ...
    if plan["type"] == "create_note":
        client.create_voice_note(scan_id=plan["patient_id"],
                                 transcription=plan["note_draft"]["content"])
...
def ask_clarification_node(state: GraphState) -> GraphState:
    return state  # response_text already set by resolve_patient_node
...
def build_patient_summary(dossier: dict) -> str:
    ...
    scans = dossier.get("scans", [])
    meds  = dossier.get("medications", [])
    appts = dossier.get("appointments", [])

    last_scan = "none on record"
    if scans:
        s = scans[0]
        last_scan = (f"{s.get('modality','')} on {str(s.get('uploaded_at',''))[:10]}, "
    ...
    if appts:
        a = appts[0]
        last_appt = f"{a.get('type','')} on {str(a.get('scheduled_at',''))[:10]}"
...
class BackendClient:
    ...
    def get_patient_dossier(self, patient_id: int) -> dict:
        return self._get(f"/doctor/patients/{patient_id}")
    ...
    def create_medication(self, patient_id: int, data: dict) -> dict:
        return self._post("/medications", {"patientId": patient_id, **data})

    def create_appointment(self, patient_id: int, doctor_id: str, data: dict) -> dict:
...
async def entrypoint(ctx: JobContext):
    ...
    doctor_id    = participant.identity   # Clerk user ID passed from frontend
    ...
    async def on_transcript(transcript: str):
        ...
        # Sync session from graph result
...
[ ]  4. Implement core/backend_client.py (search_patients, get_patient_dossier)
...
[ ]  9. Implement safety/validator.py
...
[ ] 15. Add frontend/hooks/useVoiceAgent.ts
[ ] 16. Add frontend/components/voice/VoiceAgentButton.tsx
[ ] 17. Add VoiceAgentButton to app/doctor/layout.tsx
[ ] 18. End-to-end test: "Open Rahul Verma and summarize the last visit"
```
...
[ ]  2. Add graph nodes: run_prescription_agent, stage_for_confirmation,
                          execute_pending, cancel_pending
[ ]  3. Update graph edges for update_medication intent
[ ]  4. Add backend_client: get_active_medications, create_medication
[ ]  5. Add validator rules for prescription draft
[ ]  6. End-to-end test: "Continue amoxicillin 500mg twice daily for 5 days"
```
...
[ ]  4. Add backend_client: create_appointment, send_message
